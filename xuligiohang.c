...
// hàm trả về 1 nếu điều chỉnh thành công và 0 nếu không ìuithành công // (ID không tìm thấy)
int reduceItemQuantityByID(Item** head, char ID[], 
  int reduceQuantity) {// SINH VIÊN CODE TIẾP VÀO ĐÂY
  Item* p = *head;
  if (p == NULL) return 0;
  if ( strcmp(p->ID,ID) == 0 ){
  	if ( p->quantity  > reduceQuantity) p->quantity -= reduceQuantity;
  	else  {
  		*head = p->next;
  		free(p);
	  }
  ...
  while (p->next!=NULL){
  	if (strcmp(p->next->ID,ID) == 0) {
  		if (p->next ->quantity > reduceQuantity) p->next -> quantity -= reduceQuantity;
  		else {
  			Item* temp = p->next;
  			p->next = temp->next;
  			free(temp);
...
void addNewItems(Item** head) {
    ...
    while (1) {
        ...
        fgets(nextLine, sizeof(nextLine), stdin);
        nextLine[strcspn(nextLine, "\r\n")] = 0;
        strcpy(insertDate, nextLine);fgets(nextLine, sizeof(nextLine), stdin);
        nextLine[strcspn(nextLine, "\r\n")] = 0;
        sscanf(nextLine, "%lf", &price);
