...
{
	// Code tại đây
    TNODE* pNode = findKey(root,parentLabel);
    if (pNode!=NULL) {
        if (pNode->firstChild==NULL) pNode->firstChild = new TNODE{newChildLabel, NULL,NULL};
        else 
        {   
            TNODE *currChild = pNode->firstChild;
            while(currChild->nextSibling != NULL) currChild = currChild->nextSibling;
            currChild->nextSibling = new TNODE{newChildLabel, NULL ,NULL};
        }
    }
}
...
{
	// Code tại đây
    TNODE *nodeXoa = findKey(root,branchRootLabel);
    if (nodeXoa!=NULL){
    TNODE *parentNode = findParent(root,branchRootLabel);
        if (parentNode!=NULL){
            TNODE *currChild = parentNode->firstChild;
            if (currChild->info == branchRootLabel) parentNode->firstChild = currChild->nextSibling;
            else{
                while ( (currChild->nextSibling)->info != branchRootLabel) currChild = currChild->nextSibling;
            }
            currChild->nextSibling = nodeXoa->nextSibling;
            deleteTree(&nodeXoa);
            nodeXoa = NULL; 
        }
    }
...
int countTotalNodes(TNODE* root)
{
    ...
    int totalChild ;
    while (currChild!=NULL){
        totalChild = countTotalNodes(currChild);
        sum += totalChild;
        currChild = currChild->nextSibling;
    }
    return sum + 1;
...
int countTotalLeaves(TNODE* root)
{
    ...
    int totalChild ;
    while (currChild!=NULL){
        totalChild = countTotalLeaves(currChild);
        sum += totalChild;
        currChild = currChild->nextSibling;
    }
    return sum;
